...
// 视图设置面板的原始配置（未经过环境变量与总开关解析）
// 供管理后台读取默认值；站点运行时请使用下方经过解析的 displaySettingsConfig
export const displaySettingsConfigDefaults: DisplaySettingsConfig = {
	// ── 总开关 (Master switch) ────────────────────────────
...
export const displaySettingsConfig: DisplaySettingsConfig =
	resolveDisplaySettingsConfig(
		mergeUserConfig("displaySettings", displaySettingsConfigDefaults),
	);
