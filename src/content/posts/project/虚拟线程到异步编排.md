...
description: 记录虚拟线程到异步编排的详细分析，包括为什么要有这些技术、虚拟线程的实现原理、异步编排的使用场景、常见问题、最佳实践等。
tags: [JAVA,多线程, 虚拟线程,异步编排,技术]
category: 项目
draft: false
---
...
- [ ] JDK 21+ 环境已就绪
- [ ] Spring Boot 3.2+ 已配置 `spring.threads.virtual.enabled=true`
- [ ] 已将 `synchronized` 块替换为 `ReentrantLock`（JDK 21/23）
- [ ] 没有对虚拟线程建线程池
- [ ] ThreadLocal 使用已审查，大对象已改用 ScopedValue
- [ ] 数据库连接池大小已配置，并用 Semaphore 限制并发
- [ ] 已添加 `-Djdk.tracePinnedThreads=full` 检测 Pinning
- [ ] JDBC 驱动已升级到兼容虚拟线程的版本
- [ ] CPU 密集任务仍使用专用平台线程池
- [ ] 所有并发任务都正确处理了中断

---

_参考资料：JEP 444（Virtual Threads）· JEP 453（Structured Concurrency）· 《Java 并发编程实战》Brian Goetz · Project Loom 官方文档 · Spring Boot 3.2 Release Notes_