...
function settings(options: Record<string, unknown>): CloudTrainSettings {
	return {
		speed: number(options.speed, 1, 0, 5),
		...
		detail: Math.round(number(options.detail, 8, 1, 8)),
	...
	#start(): (() => void) | undefined {
		...
		try {
			...
			function draw(now: number) {
				...
				if (!s.introEnabled || media.matches) introProgress = 1;
					...
					introProgress = Math.min(
						1,
						introProgress +
							Math.min((now - (last || now)) / 1000, 0.05) / s.introDuration,
					);
			...
			const reset = () => {
				...
				last = 0;
				history = false;
				if (scale !== state.current.resolution) {
					scale = state.current.resolution;
					updatePixelSize();
				}
				request();
